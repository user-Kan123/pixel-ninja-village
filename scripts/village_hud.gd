...
func _draw() -> void:
	...
	## 右上：标题
	draw_string(font, Vector2(v.x - 24.0, 34.0), Data.s("hud.title"), HORIZONTAL_ALIGNMENT_RIGHT, -1, 13, COL_DIM)
	## 中下：交互提示
	...
	## 底部操作提示
	draw_string(font, Vector2(v.x - 24.0, v.y - 18.0), Data.s("village.hint"), HORIZONTAL_ALIGNMENT_RIGHT, -1, 12, Color(1, 1, 1, 0.45))
	## 通知
	if notice_timer > 0.0:
