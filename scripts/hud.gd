...
func _draw_counters(font: Font) -> void:
	...
	if player.buff_timer > 0.0 and not player.buff_id.is_empty():
		...
		draw_string(font, Vector2(size_v.x - 24.0, 58.0), "%s %0.1fs" % [Data.s(String(cfg.get("name_key", player.buff_id))), player.buff_timer], HORIZONTAL_ALIGNMENT_RIGHT, -1, 13, Color(0.6, 0.9, 1.0))
	if player.combo_count >= 2:
		var t: float = clampf(player.combo_timer / 1.2, 0.0, 1.0)
		draw_string(font, Vector2(size_v.x - 24.0, 92.0), "%s x %d" % [Data.s("hud.combo"), player.combo_count], HORIZONTAL_ALIGNMENT_RIGHT, -1, 26, Color(1.0, 0.85, 0.3, 0.4 + 0.6 * t))
